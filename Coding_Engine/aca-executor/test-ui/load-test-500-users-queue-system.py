#!/usr/bin/env python3
"""
Load Test: 500 Users with Queue System
Sends all 500 requests at once - queue system handles them
Each user: 1 DSA question with 20 test cases (including 3 long test cases)
Queue system: 8 concurrent per replica, remaining requests queued
...
# DSA Question: Sum of Array Elements (same as before)
DSA_QUESTION = {
    "python": {
        "code": """n = int(input())
...
def submit_request(user_id, language, question_data):
    """Submit a request for a single user - queue system will handle it"""
    ...
    try:
        # Execute curl command with reasonable timeout
        curl_cmd = [
            'curl', '-s', '-w', '\nHTTP_CODE:%{http_code}\nTIME_TOTAL:%{time_total}',
            '-X', 'POST',
            '-H', 'Content-Type: application/json',
            '-d', f'@{temp_file}',
            '--max-time', '30',  # 30 second timeout (should be enough for 20 test cases)
            API_URL + '/runall'
        ]
...
def main():
    ...
    print("=" * 120)
    print("🚀 LOAD TEST: 5 Users (Quick Test)")
    print("=" * 120)
    print(f"API URL: {API_URL}")
    print(f"Total Users: {num_users}")
    print(f"Queue System: 8 concurrent per replica, remaining requests queued")
    print(f"Configuration: 2 ready replicas = 16 concurrent, max 3 replicas = 24 concurrent")
    ...
    print("⚡ Sending 150 requests in batches...")
    print("   Queue system will handle them (8 concurrent per replica)")
    print("   Remaining requests will be queued and processed automatically")
    ...
    # Send all 5 requests at once (small test)
    batch_size = 5  # Small test - all at once
    batch_delay = 1  # 1 second delay (not needed for 5 users)
    
    print("📤 Submitting requests in batches to queue...")
    print(f"   Batch size: {batch_size} requests")
    print(f"   Batch delay: {batch_delay} seconds")
    print(f"   Total batches: {math.ceil(num_users / batch_size)}")
    print("⏳ Queue processes 16 concurrent at a time (2 replicas × 8)")
    print()
    ...
    with ThreadPoolExecutor(max_workers=batch_size) as executor:
        ...
        for i in range(0, num_users, batch_size):
            ...
            batch_users = user_assignments[i:i + batch_size]
            
            print(f"⚡ Batch {batch_num}/{math.ceil(num_users / batch_size)}: Sending {len(batch_users)} requests...")
            
            # Submit batch
            batch_futures = {
                executor.submit(submit_request, user_id, language, question_data): user_id
                for user_id, language, question_data in batch_users
            }
            all_futures.extend(batch_futures.items())
            
            # Wait a bit before next batch (unless it's the last batch)
        ...
        for future, user_id in all_futures:
            completed += 1
            result = future.result()
            results.append(result)
            
            status_icon = "✅" if result['success'] else "❌"
            if completed % 15 == 0 or not result['success']:
                print(f"   [{completed:3d}/{num_users}] {status_icon} {result['user_id']:10s} - {result['language']:10s} - {result['http_code']:3s} - {result['wait_time_ms']:8d}ms - TC: {result['passed_count']}/{result['total_count']}")
    ...
    output_file = "LOAD_TEST_5_USERS_QUICK_RESULTS.csv"
    ...
    print(f"   Total Duration: {overall_time_ms}ms ({overall_time:.2f}s / {overall_time/60:.2f} minutes)")
    ...
    if successful:
        ...
        exec_times = [r['exec_time_ms'] for r in successful if r['exec_time_ms'] > 0]

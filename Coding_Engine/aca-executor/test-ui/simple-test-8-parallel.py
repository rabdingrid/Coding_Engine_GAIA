#!/usr/bin/env python3
"""
Simple Test: 8 Users in PARALLEL with /runall endpoint
Test concurrent capacity of one replica (should handle 8 concurrent)
"""
...
def main():
    print("=" * 80)
    print("🧪 PARALLEL TEST: 8 Users at ONCE with /runall")
    print("=" * 80)
    print(f"API: {API_URL}")
    print(f"Endpoint: /runall")
    print(f"Test Cases: 3 per user")
    print(f"Concurrency: 8 users at once (testing replica capacity)")
    ...
    # Test 8 users in PARALLEL (all at once)
    print("📤 Sending 8 requests in PARALLEL...")
    print()
    
    results = []
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(test_user, i): i for i in range(1, 9)}
        
        for future in as_completed(futures):
            result = future.result()
            results.append(result)
            status = "✅" if result['success'] else "❌"
            print(f"{status} User {result['user_id']:2d}: {result['start_ts']} → {result['end_ts']} ({result['duration_ms']:5d}ms) - {result['http_code']} - TC: {result['passed']}/{result['total']}")
    ...
    print(f"❌ Failed: {8 - len(successful)}/8")
    print()
    ...
    print(f"⏱️  Total Time (all 8 parallel): {int(overall_time * 1000)}ms ({overall_time:.2f}s)")

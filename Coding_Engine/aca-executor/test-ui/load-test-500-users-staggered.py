#!/usr/bin/env python3
"""
Load Test: 500 Dummy Users with STAGGERED Requests (Better Performance)
...
Staggers requests in batches to prevent overwhelming the system
...
def main():
    ...
    print("=" * 120)
    print("🚀 LOAD TEST: 500 Dummy Users with STAGGERED Requests (Better Performance)")
    print("=" * 120)
    print(f"API URL: {API_URL}")
    print(f"Total Users: {num_users}")
    print(f"Batch Size: {batch_size} (staggered to prevent overwhelming)")
    ...
    print("⚡ Starting STAGGERED execution of 500 users...")
    ...
    for batch_num in range(num_batches):
        ...
        batch_end = min((batch_num + 1) * batch_size, num_users)
        batch_assignments = user_assignments[batch_start:batch_end]
        
        print(f"📦 Batch {batch_num + 1}/{num_batches} ({len(batch_assignments)} users)...")
        
        # Submit batch requests
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            futures = {
                executor.submit(submit_request, user_id, language, question_data): user_id
                for user_id, language, question_data in batch_assignments
            }
            ...
            for future in as_completed(futures):
                ...
                if completed % 25 == 0 or not result['success']:
        ...
        # Wait a bit between batches to let system recover
    ...
    output_file = "LOAD_TEST_500_USERS_STAGGERED_RESULTS.csv"

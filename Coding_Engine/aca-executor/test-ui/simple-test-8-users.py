#!/usr/bin/env python3
"""
Simple Test: 8 Users with /runall endpoint
Just execute code and show timestamps - no database, no complexity
"""
...
def main():
    print("=" * 80)
    print("🧪 SIMPLE TEST: 8 Users with /runall")
    print("=" * 80)
    print(f"API: {API_URL}")
    ...
    # Test 8 users sequentially (one after another)
    print("📤 Sending 8 requests sequentially...")
    print()
    
    results = []
    for i in range(1, 9):
        print(f"   User {i}...", end=" ", flush=True)
        result = test_user(i)
        results.append(result)
        
        status = "✅" if result['success'] else "❌"
        print(f"{status} {result['start_ts']} → {result['end_ts']} ({result['duration_ms']}ms) - {result['http_code']} - TC: {result['passed']}/{result['total']}")
    ...
    print(f"⏱️  Total Time: {int(overall_time * 1000)}ms ({overall_time:.2f}s)")
    print()

...
# Test script for "Compress Word" problem (Question ID: 622797) - Java HackerRank Style
# Tests all 15 test cases from the 622797 directory using HackerRank-style Java boilerplate
...
echo "🧪 Testing: Compress Word Problem (Java HackerRank Style) - Question ID: 622797"
echo "📍 URL: $CONTAINER_APP_URL"
echo "📁 Test Cases Directory: $TEST_CASES_DIR"
echo ""

# Use Python to create the JSON payload properly
python3 << 'PYEOF' > /tmp/test_compress_java_hr_payload.json
import json
import os
...
payload = {
    ...
    "user_id": "test_compress_word_java_hr",
    "question_id": "622797"
}
...
RESPONSE=$(curl -s -m 60 -X POST "$CONTAINER_APP_URL/execute" \
  ...
  -d @/tmp/test_compress_java_hr_payload.json)
END_TIME=$(date +%s)
TOTAL_TIME=$((END_TIME - START_TIME))

# Save response
echo "$RESPONSE" > /tmp/test_compress_java_hr_response.json
...
try:
    with open('/tmp/test_compress_java_hr_response.json', 'r') as f:
        d = json.load(f)
...
# Cleanup
rm -f /tmp/test_compress_java_hr_payload.json /tmp/test_compress_java_hr_response.json

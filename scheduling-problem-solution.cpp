...
int calculateSchedulingTime(vector<int>& capacity, long long requests) {
    // Note: Changed signature to accept vector<int>& instead of int capacity[]
    // because main() uses vector<int>
    ...
    while (remaining > 0) {
        if (pq.empty()) {
            // Should not happen based on constraints, but safety check
            break;
        }

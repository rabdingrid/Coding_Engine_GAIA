...
async def get_db_pool():
    """Lazy initialization of database connection pool - only when /submit is called"""
    global db_pool
    if db_pool is None:
...
async def lifespan(app: FastAPI):
    ...
    # No database initialization on startup - lazy init only when needed
...
def execute_java(code: str, stdin: str = "", timeout: int = EXECUTION_TIMEOUT):
    ...
    temp_dir = create_sandboxed_directory()
    start_time = time.time()  # Initialize at the start to avoid UnboundLocalError
    cpu_usage = 0.0
    memory_usage = 0
...
async def save_submission_to_db(
    ...
    """Save submission results to database (lazy connection)"""
...
@limiter.limit("1000 per minute")  # Increased for high load testing (500 users)
async def runall(request: Request, runall_req: RunAllRequest):
    ...
    try:
        ...
        # Execute test cases sequentially (like old Flask version)
        # Multiple users run in parallel, but each user's test cases run sequentially
        for idx, test_case in enumerate(test_cases):
            test_input = test_case.input
            expected_output = test_case.expected_output
            test_case_id = test_case.id or f'test_{idx + 1}'
        ...
        # Performance logging: Test execution complete
        ...
        logger.info(f"⏱️ Sequential test execution completed: {test_execution_time}ms for {len(test_cases)} test cases (avg: {avg_time:.1f}ms per test)")
...
@limiter.limit("200 per minute")  # Increased for high load testing
async def submit(request: Request, submit_req: SubmitRequest):
    ...
    try:
        ...
        # Execute test cases sequentially (parallel execution has thread-safety issues with network blocking)
        for idx, test_case in enumerate(test_cases):
            test_input = test_case.input
            expected_output = test_case.expected_output
            test_case_id = test_case.id or f'test_{idx + 1}'
        ...
        # Save to database (lazy connection - only when /submit is called)
        submission_id = None
        ...
        try:
            ...
            # Continue even if DB save fails - don't block response
            db_save_time = 0
        
        # Final performance summary
        total_time = int((time.time() - start_time) * 1000)
        overhead = total_time - test_execution_time - (db_save_time if 'db_save_time' in locals() else 0)
        logger.info(f"📊 Total /submit time: {total_time}ms | Tests: {test_execution_time}ms | DB: {db_save_time if 'db_save_time' in locals() else 0}ms | Overhead: {overhead}ms")
        ...
        response = ExecutionResponse(
            ...
            metadata={
                ...
                'saved_to_db': submission_id is not None if 'submission_id' in locals() else False
            },
            timestamp=timestamp
        )
